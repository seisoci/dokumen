...
// Backend css backend (single page use)
(glob.sync('resources/backend/css/**/!(_)*.css') || []).forEach(file => {
    file = file.replace(/[\\\/]+/g, '/');
    mix.copy(file, file.replace('resources/backend/css', 'public/css/backend').replace(/\.css$/, '.css'));
});

// Backend sass backend (single page use)
(glob.sync('resources/backend/sass/**/!(_)*.scss') || []).forEach(file => {
    file = file.replace(/[\\\/]+/g, '/');
    mix.sass(file, file.replace('resources/backend/sass', 'public/css/backend').replace(/\.scss$/, '.css'));
});
